...
class CuLevelInit extends Base
{

    public $is_show = [1 => '是', 2 => '否'];//显示
    public $star    = [1 => '准星级', 2 => '二星级', 3 => '三星级', 4 => '四星级', 5 => '五星级', 6 => '六星级', 7 => '七星级', 8 => '私人银行级'];//星级:1准星级,2二星级,3三星级,4四星级,5五星级,6六星级,7七星级,8私人银行级
    public $level   = [1 => '战略客户', 2 => '重点客户', 3 => '价值客户', 4 => '一般客户', 5 => '长尾客户'];//客户分层:1战略客户,2重点客户,3价值客户,4一般客户,5长尾客户
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        $item['star_name']    = $this->star[$item['star']];//星级:1准星级,2二星级,3三星级,4四星级,5五星级,6六星级,7七星级,8私人银行级
        $item['level_name'] = $this->level[$item['level']];//客户分层:1战略客户,2重点客户,3价值客户,4一般客户,5长尾客户
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            $result                = $CuLevelModel->where($where)->strict(false)->update($params);
            //if ($result) $result = $item["id"];
        } elseif (!empty($params["id"])) {
            //如传入id,根据id编辑数据
            $params["update_time"] = time();
            $result                = $CuLevelModel->where("id", "=", $params["id"])->strict(false)->update($params);
            //if($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $CuLevelModel->strict(false)->insert($params, true);
        }
    ...
    public function edit_post_two($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $CuLevelModel->strict(false)->insert($params);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $CuLevelModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $CuLevelModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $CuLevelModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $CuLevelModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $result = $CuLevelInit->get_list($where, $params);
        ...
        $Excel->excelExports($result, $headArrValue, ["fileName" => "客户层级"]);
    }
