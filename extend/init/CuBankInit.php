...
class CuBankInit extends Base
{

    public $is_deposit =[1=>'是',2=>'否'];//存款银行 
public $is_loan =[1=>'是',2=>'否'];//贷款银行 
public $is_level =[1=>'是',2=>'否'];//分层统计 
public $is_show =[1=>'是',2=>'否'];//显示 
    ...
    protected $DataFormat    = "find";//数据格式,find详情,list列表
        ...
        public function common_item($item = [], $params = [])
        {
            ...
            /** 处理文字描述 **/
            $item['is_deposit_name']=$this->is_deposit[$item['is_deposit']];//存款银行 
$item['is_loan_name']=$this->is_loan[$item['is_loan']];//贷款银行 
$item['is_level_name']=$this->is_level[$item['is_level']];//分层统计 
    ...
    */
    public function get_join_list($where=[], $params = [])
    {
        ...
        */
        public function edit_post($params,$where=[])
        {
          ...
          if(!empty($where)){
                ...
                $result                = $CuBankModel->where($where)->strict(false)->update($params);
                //if ($result) $result = $item["id"];
            } elseif (!empty($params["id"])) {
                //如传入id,根据id编辑数据
                $params["update_time"] = time();
                $result                = $CuBankModel->where("id","=",$params["id"])->strict(false)->update($params);
                //if($result) $result = $item["id"];
           } else {
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $CuBankModel->strict(false)->insert($params,true);
            }
        ...
        */
        public function edit_post_two($params,$where=[])
        {
            ...
            if(!empty($where)){
                ...
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $CuBankModel->strict(false)->insert($params);
            }
        ...
        */
        ...
        if ($type == 1) $result = $CuBankModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $CuBankModel->destroy($id,true);//真实删除
        ...
        */
        public function batch_post($id, $params=[])
        {
            ...
            $params["update_time"] = time();
            $result = $CuBankModel->where($where)->strict(false)->update($params);//修改状态
        ...
        */
        public function list_order_post($list_order,$params=[])
        {
            ...
            foreach ($list_order as $k => $v) {
                $where   = [];
                $where[] = ["id", "=", $k];
                $result  = $CuBankModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
            }
        ...
        */
        public function export_excel($where = [], $params = [])
        {
            ...
            $Excel->excelExports($result, $headArrValue, ["fileName" => "银行管理"]);
        }
