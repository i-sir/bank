...
class CustomerController extends AuthController
{
    ...
    public function edit_customer()
    {
        ...
        $params                 = $this->request->param();
        $params["user_id"]      = $this->user_id;
        $params["subbranch_id"] = $this->user_info['subbranch_id'];
        ...
        if (!empty($store_bank_list) && !empty($loan_bank_list)) {
            ...
            // 设置is_credit参数
            $params['is_credit'] = $has_credit ? 1 : 2;
        } else {
     ...
     * @OA\Post(
     *     tags={"客户信息记录"},
     *     path="/wxapp/customer/find_admin_statistics",
     *
     *
     *
     *    @OA\Parameter(
     ...
     *
     */
    public function find_admin_statistics()
    {
        $this->checkAuth(); // 权限校验：验证当前用户是否有权限访问该统计接口
