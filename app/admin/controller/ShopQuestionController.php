 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\ShopQuestionController();
 * )
 */
...
class ShopQuestionController extends AdminBaseController
{
    ...
    protected function base_edit()
    {
        ...
        $ShopQuestionInit = new \init\ShopQuestionInit();//shop_question     (ps:InitController)
        $this->assign('type_list', $ShopQuestionInit->type);
        $this->assign('question_type_list', $ShopQuestionInit->question_type);
    }
    ...
    public function index()
    {
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $ShopQuestionInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $ShopQuestionInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 检测参数信息 **/
        $validateResult = $this->validate($params, 'ShopQuestion');
        if ($validateResult !== true) $this->error($validateResult);


        /** 插入数据 **/
        $result = $ShopQuestionInit->admin_edit_post($params);
    ...
    public function find()
    {
        ...
        $result                  = $ShopQuestionInit->get_find($where, $params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $ShopQuestionInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $ShopQuestionInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function audit_post()
    {
        ...
        $params["InterfaceType"] = "admin";//接口类型
        $params["DataFormat"]    = "find";//数据格式,find详情,list列表
        $item                    = $ShopQuestionInit->get_find($where);
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $ShopQuestionInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $ShopQuestionInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $ShopQuestionInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
