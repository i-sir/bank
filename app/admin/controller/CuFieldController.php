 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\CuFieldController();
 * )
 */
...
class CuFieldController extends AdminBaseController
{
    ...
    protected function base_edit()
    {
        ...
        $CuFieldInit = new \init\CuFieldInit();//cu_field     (ps:InitController)
        $this->assign('type_list', $CuFieldInit->type);
    }
    ...
    public function index()
    {
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $CuFieldInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $CuFieldInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 插入数据 **/
        $result = $CuFieldInit->admin_edit_post($params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $CuFieldInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $CuFieldInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function audit_post()
    {
        ...
        $params["InterfaceType"] = "admin";//接口类型
        $params["DataFormat"]    = "find";//数据格式,find详情,list列表
        $item                    = $CuFieldInit->get_find($where);
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $CuFieldInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $CuFieldInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $CuFieldInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
