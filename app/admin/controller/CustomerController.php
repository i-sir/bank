...
/**
    ...
    *     "version"             =>"1.0",
    *     "use"                 => new \app\admin\controller\CustomerController();
    * )
    */
	...
	//	}
		...
		*/
		protected function base_edit()
		{
			...
			$CustomerInit=new \init\CustomerInit();//customer     (ps:InitController)
 $this->assign('type_list', $CustomerInit->type);
		}
	...
	public function index()
	{
		...
		/** 导出数据 **/
		if ($params["is_export"]) $CustomerInit->export_excel($where, $params);


		/** 查询数据 **/
		$result = $CustomerInit->get_list_paginate($where,  $params);
		...
		public function add_post()
		{
				...
				/** 检测参数信息 **/
				$validateResult = $this->validate($params, 'Customer');
				if ($validateResult !== true) $this->error($validateResult);


				/** 插入数据 **/
				$result = $CustomerInit->admin_edit_post($params);
		...
		public function edit_post()
		{
			...
			/** 提交数据 **/
			$result = $CustomerInit->admin_edit_post($params,$where);
			if (empty($result)) $this->error("失败请重试");
		...
		public function edit_post_two()
		{
			...
			/** 提交数据 **/
			$result = $CustomerInit->edit_post_two($params,$where);
			if (empty($result)) $this->error("失败请重试");
		...
		public function audit_post()
		{
			...
			$params["InterfaceType"] = "admin";//接口类型
			$params["DataFormat"]    = "find";//数据格式,find详情,list列表
			$item = $CustomerInit->get_find($where);
		...
		public function delete()
		{
			...
			/** 删除数据 **/
			$result = $CustomerInit->delete_post($id);
			if (empty($result)) $this->error("失败请重试");
		...
		public function batch_post()
		{
			...
			//提交编辑
			$result = $CustomerInit->batch_post($id, $params);
			if (empty($result)) $this->error("失败请重试");
		...
		public function list_order_post()
		{
			...
			//提交更新
			$result = $CustomerInit->list_order_post($params);
			if (empty($result)) $this->error("失败请重试");
